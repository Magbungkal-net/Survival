     ...
     after discord user role changes:
      ...
      - if <[added.role].parse[id].contains[1131182921915564032]>:
         - definemap magbungkal_booster_map:
             ...
             description: "> All rewards was given to you in-game [**<server.match_offline_player[<[ign]>].name>**]. If you didn't recieve it please create a ticket immediately.<n><n>> » 1500 Coins<n>> » ₱50,000<n>> » 32 Vote Keys<n><n>There are now **<[booster.role].users.size>** boosters in Magbungkal.net Discord"
         ...
         - execute as_server "money give <server.match_offline_player[<[ign]>].name> 50k"
         - execute as_server "crate key give <server.match_offline_player[<[ign]>].name> vote 15"
         - execute as_server "lp user <server.match_offline_player[<[ign]>].name> permission set mcpets.wumpus"

DiscordBoosterRewardsCommand:
    type: command
    name: booster
    description: Booster
    usage: /booster [playerName]
    permission: denizen.admin
    debug: false
    script:
        - define args <context.args>
        - execute as_server "a <&f><&l><[args].get[1]> <&f>Earn 1,500 Coins, ₱50,000, and 15 Vote Keys by boosting Magbungkal Discord (/discord)"
        - execute as_server "magbungkalcoins give <[args].get[1]> 1500"
        - execute as_server "money give <[args].get[1]>  50k"
        - execute as_server "crate key give <[args].get[1]>  vote 15"
        - execute as_server "lp user <[args].get[1]> permission set mcpets.wumpus"