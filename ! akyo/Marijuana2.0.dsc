...
SeedPop:
  ...
  - runlater Veg1 delay:140t def:<[Plant]>
  - wait 1
...
Veg1:
  ...
  - if <[Plant].flag[Growquality]> < -1:
    - equip <[Plant]> head:Dead_Plant_item
    - flag <[Plant]> DeadPlant
  - if <[Plant].flag[Nutes]> >= 0 and <[Plant].flag[Nutes]> < 5:
    - equip <[Plant]> head:Veg2_item
    - flag <[Plant]> GrowQuality:+:2
    - flag <[Plant]> Stage:Veg
    - runlater Veg2 delay:140t def:<[Plant]>
  - else if <[Plant].flag[Nutes]> > 5:
    - flag <[Plant]> GrowQuality:--
    - runlater Veg2 delay:140t def:<[Plant]>
    - runlater Veg2 delay:140t def:<[Plant]>
...
Veg2:
  ...
  - if <[Plant].flag[Nutes]> >= 2 and <[Plant].flag[Nutes]> < 8:
    ...
    - flag <[Plant]> GrowQuality:+:3
    - runlater Veg3 delay:140t def:<[Plant]>
...
Veg3:
  ...
  - if <[Plant].flag[Nutes]> >= 6 and <[Plant].flag[Nutes]> < 10:
    ...
    - flag <[Plant]> GrowQuality:+:3
    - runlater Veg4 delay:140t def:<[Plant]>
...
Veg4:
  ...
  - else if  <[Plant].location.light> > 13:
      - flag <[Plant]>  GrowQuality:++
  - if <[Plant].flag[Nutes]> >= 8 and <[Plant].flag[Nutes]> <= 12:
    - equip <[Plant]> head:Bloom1_item
    - flag <[Plant]> GrowQuality:+:3
    - flag <[Plant]> stage:Bloom1
    - runlater Bloom1 delay:140t def:<[Plant]>
  - else if <[Plant].flag[Nutes]> < 8:
    - flag <[Plant]> GrowQuality:--
    - runlater Veg4 delay:140t def:<[Plant]>
  - else if <[Plant].flag[Nutes]> > 12:
    - flag <[Plant]> GrowQuality:-:3
    - runlater Bloom1 delay:140t def:<[Plant]>
Bloom1:
  type: task
  ...
  - if <[Plant].flag[Nutes2]> >= 1 and <[Plant].flag[Nutes2]> < 4 :
    ...
    - flag <[Plant]> GrowQuality:+:2
    - runlater Bloom2 delay:140t def:<[Plant]>
...
Bloom2:
  ...
  - if <[Plant].flag[Nutes2]> >= 4 and <[Plant].flag[Nutes2]> < 8 :
    ...
    - flag <[Plant]> GrowQuality:+:2
    - runlater Bloom3 delay:140t def:<[Plant]>
...
Bloom3:
  ...
  - if <[Plant].flag[Nutes2]> >= 6 and <[Plant].flag[Nutes2]> < 10 :
    ...
    - flag <[Plant]> GrowQuality:+:2
    - runlater Bloom4 delay:140t def:<[Plant]>
...
RightClickPlantEvent:
    ...
    debug: false
    events:
        on player right clicks armor_stand with:NuteChecker:
        ...
        - if <context.entity.flag[Stage]> != veg:
          ...
          - narrate "Nute level:<context.entity.flag[nutes2]>" format:serverformat
          - narrate "Plant Stage: <context.entity.equipment_map.get[helmet].display>" format:serverformat
          - stop
        - else if <context.entity.has_flag[Nutes]>:
          - determine passively cancelled
          - narrate "Nute level:<context.entity.flag[nutes]>" format:serverformat
          - narrate "Plant Stage: <context.entity.equipment_map.get[helmet].display>"
          - stop
...
Raygun:
  ...
  script:
  - cast slow amplifier:2 duration:5s <[hit_entities]>
  - hurt 8 <[hit_entities]>
...
NuteChecker:
  type: item
  material: Clock
  display name: NuteChecker
  lore:
  - Right Click to Check NuteLevel
  recipes:
      1:
       type: shapeless
       output_quantity: 1
       input: Clock|MarijuanaBible|
SeededPot:
  type: item
  material: Stick
  mechanisms:
...
MarijuanaBible:
  ...
  - You need to water it 2 times everytime you see the plant change stages (every 2 minutes!) once the plant begins to bloom switch to the Bloomjug. When ready to harvest simply right click with shears
  - To cure the plant place Buds in a barrel and check on them every 2 minutes Combine This Book with a Clock to make A NuteChecker
