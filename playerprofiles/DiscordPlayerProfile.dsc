    ...
    events:
        ...
        - if <[text].starts_with[<[p_profile_cmd]>]>:
            ...
            - if <[player].if_null[0]> == 0:
                - define message_map <script[discord_player_profile_config].parsed_key[messages].get[player-not-online]>
                - define embed <discord_embed.with_map[<[message_map]>]>
                - ~discordmessage id:magbungkal reply:<[message]> <[embed]>
                - stop
            - if <[player].if_null[0]> == 0:
                - define message_map <script[discord_player_profile_config].parsed_key[messages].get[player-uknown]>
                - define embed <discord_embed.with_map[<[message_map]>]>
                - ~discordmessage id:magbungkal reply:<[message]> <[embed]>
                - stop
